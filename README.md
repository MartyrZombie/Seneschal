...
A single command principal (the owner) issues instructions. The agent runs as an unprivileged Linux user with deterministic enforcement (ingest, dispatcher, broker, egress) and model-backed orchestration (Qwen3-30B-A3B via llama.cpp on Tesla P40).
